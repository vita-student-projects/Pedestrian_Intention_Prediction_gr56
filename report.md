...
##### Model Modification and Adaptation for Training & Testing
...
&emsp; A new inference file was created to make predictions on any video, and to save the results in a json file. This file was also modified to take into account the new Dataset subclasses, and a new specific configuration file *inference.yaml* previously created to store the network specifications and choosen inference parameters.

- New configuration files
- New function to train the model, record the metrics, create checkpoints, validation, evaluation, F1 - train_jaad.py
- change input dimension to accept bbox and kp
- change output dimension to predict crossing/not crossing
- Inference class
- json file output for inference & fit to bboxes
